...
import DownloadBt from "../resume/DownloadBt";

const Step3report = ({ report, user, setUser }) => {
    ...
    <div className="min-h-screen bg-white flex items-center justify-center md:p-5">
      ...
      >
        <div className="border-b border-white/10 px-8 py-6 flex items-center justify-between">
          <div>
            <div
              ...
              className="inline-flex items-center gap-2 text-white rounded-full border border-black/20 bg-black px-3 py-1.5 cursor-pointer"
            >
              <FiArrowLeft size={14} />
            ...
            <DownloadBt docRef={reportRef} user={user} setUser={setUser} />
          </div>
        ...
        <div className="p-8" ref={reportRef}>
            ...
            <div className="rounded-2xl bg-[#17181e] border border-green-500/20 p-6">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-lg bg-green-500 flex items-center justify-center">
                  <FiTrendingUp className="text-white" size={15} />
                </div>
              ...
              <div className="mt-5 space-y-3">
                ...
                report.strengths?.map((s)=>(
                  <div key={s} className="flex items-start gap-3 rounded-xl bg-green-500/5 border border-green-500/10 p-3.5">
                    <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center flex-shrink-0 text-xs"><FiCheck size={16} />
                  </div>
                     <p className="text-sm text-zinc-300 leading-6">{s}</p>
                  </div>
                 ))
                ) : (
                  <p className="text-zinc-500 text-sm">No Stengths Available.</p>
                )}
              </div>
            </div>
              ...
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-lg bg-red-500 flex items-center justify-center">
                  <FiX className="text-white" size={15} />
                </div>
              ...
              <div className="mt-5 space-y-3">
                ...
                report.weakness?.map((s)=>(
                  <div key={s} className="flex items-start gap-3 rounded-xl bg-red-500/5 border border-red-500/10 p-3.5">
                    <div className="w-6 h-6 rounded-full bg-red-500 flex items-center justify-center flex-shrink-0 text-xs">!
                  </div>
                     <p className="text-sm text-zinc-300 leading-6">{s}</p>
                  </div>
                 ))
                ) : (
                  <p className="text-zinc-500 text-sm">No Weakness Found.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-2xl bg-[#17181e] border border-white/10 p-6">
          <h3 className="text-lg font-semibold text-whtie">Recommendation</h3>
          ...
          <div className="mt-5 space-y-3">
            {
              report.recommendations?.length > 0 ? (
                ...
                <p className="text-zinc-500 text-sm">No Recommendation Available.</p>
              )
            }
          </div>
          </div>
          ...
          <div className="mt-8">
            <h3 className="text-xl font-bold text-white mb-5">Question Wise Analysis</h3>

            <div className="space-y-5">{report.questions?.map((item,index)=>(
              <div key={index} className="rounded-2xl bg-[#17181e] border border-white/10 p-6">
                <div>
                  <span className="text-[11px] uppercase tracking-widest text-zinc-500">Question {index+1}</span>
                  <h4 className="mt-2.5 text-base font-semibold text-white leading-7">{item.questions}</h4>
                </div>
                ...
                <div className="grid md:grid-cols-4 gap-3.5 mt-6">
                  ...
                  <h5 className="mt-2 text-xl font-bold text-white">{item.feedback?.score ?? 0}</h5>
                  </div>

                   <div className="rounded-xl bg-black/20 p-3.5">
                  <p className="text-xs text-zinc-500">Clarity</p>
                  <h5 className="mt-2 text-xl font-bold text-white">{item.feedback?.clarity ?? 0}</h5>
                  </div>

                   <div className="rounded-xl bg-black/20 p-3.5">
                  <p className="text-xs text-zinc-500">Relevance</p>
                  <h5 className="mt-2 text-xl font-bold text-white">{item.feedback?.relevance ?? 0}</h5>
                  </div>

                   <div className="rounded-xl bg-black/20 p-3.5">
                  <p className="text-xs text-zinc-500">Communication</p>
                  <h5 className="mt-2 text-xl font-bold text-white">{item.feedback?.communication ?? 0}</h5>
                  </div>
                </div>
                ...
                <div className="mt-6 rounded-xl border border-green-500/20 bg-green-500/5 p-4.5">
                  ...
                  <p className="mt-2.5 text-sm leading-6 text-zinc-300">{item.feedback?.feedback || "No Feedback Available."}</p>
                </div>

                {item.feedback?.improvement?.length > 0 && (
                  <div className="mt-6">
                    <h5 className="text-sm font-semibold text-white">Suggested Improvements</h5>
                    <div className="mt-3.5 space-y-2.5">
                      {item.feedback?.improvement?.map((item,i)=>(
