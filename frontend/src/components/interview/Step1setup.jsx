...
const Step1setup = ({ user, setUser }) => {
            ...
            {[
              ...
              >
                <div className="w-7 h-7 shrink-0 rounded-lg bg-white flex items-center justify-center">
                  <FiCheck size={13} className="text-black" />
                </div>
        ...
        <div className="p-5 sm:p-7 flex flex-col">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-400">
              Configure your Interview Preference
            </p>
          </div>
  ...
  <input
    ...
    className="w-full py-2.5 rounded-xl bg-[#17181e] border border-white/10 pl-10 pr-4 text-sm text-white outline-none focus:border-white/30 transition"
  />
</div>
            </div>
              ...
              <div className="mt-1.5 flex rounded-xl bg-[#17181e] p-1 border border-white/10">
                {["technical", "hr"].map((item) => (
                  <button
                    ...
                    className={`flex-1 h-9  rounded-lg text-xs sm:text-sm font-medium capitalize transition-all cursor-pointer ${type === item ? "bg-white text-black" : "text-zinc-400 hover:text-white"}`}
                  >
                    {item}
                  </button>
                ))}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-zinc-500">
                    AI will Personalized questions using your resume.
                  </p>
                </div>
                <button
                ...
                >
                  <div
                    className={`absolute top-0.5 w-6 h-6 rounded-full bg-black transition-all ${useResume ? "left-5.5" : "left-0.5"}`}
                  />
                </button>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-semibold text-white">
                      Resume Ready <span className="text-gray-400">{resume?.suggestedRole}</span>
                    </h4>
              ...
              >
                <label className="cursor-pointer flex flex-col items-center">
                  <div className="w-11 h-11 rounded-xl bg-white flex items-center justify-center">
                    <FiUploadCloud size={20} className="text-black" />
                  </div>
                  ...
                  <p className="mt-1 text-xs text-zinc-500 text-center">
                    {resume
                      ...
                      : "pload your resume to generate personalized interview questions."}
                  </p>
                  <input
                    type="file"
                    accept=".pdf"
                    className="hidden"
                    onChange={(e) => {
                      if (e.target.value[0]) setFile(e.target.files[0]);
                    }}
                  />
                </label>
